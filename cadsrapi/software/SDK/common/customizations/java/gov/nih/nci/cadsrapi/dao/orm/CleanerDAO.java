...
public class CleanerDAO extends HibernateDaoSupport {
	...
	public void clean()
	{
		...
		Query updateActiveCarts = session.createQuery("update gov.nih.nci.cadsr.objectcart.domain.Cart set expirationDate = :nowPlusExpirationInterval" +
		...
		String initializeSessionCartSql = "UPDATE cart c" +
		...
		String nonEmptyCartSql = "UPDATE cart c  " +
		" set expiration_Date = "+fullExpirationSQL+" where" +
		" (c.user_Id like 'PublicUser%') and " +
		" (c.last_write_date > (SYSDATE - INTERVAL '"+ (sleepTime * 2)+"' MINUTE) OR c.last_read_date > (SYSDATE - INTERVAL '"+ (sleepTime * 2) +"' MINUTE)) and" +
		...
		Query deleteCartQuery = session.createQuery("delete from gov.nih.nci.cadsr.objectcart.domain.Cart " +

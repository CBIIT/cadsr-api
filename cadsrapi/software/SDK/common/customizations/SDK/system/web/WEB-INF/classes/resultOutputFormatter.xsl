...
<!--								<tr>
										...
										<table width="100%" height="50" border="0" cellspacing="0"
											...
											<tr>
												<td height="50" align="left">
													<a href="#">
														<img src="images/cadsrLogoSmall.gif" alt="Application Logo"
															hspace="10" border="0" />
													</a>
												</td>
											</tr>
																...
																<table border="0" bordercolor="orange" summary=""
																	...
																	<tr>
																		<td class="dataTablePrimaryLabel" height="20"
																			...
																			Criteria:
																			<xsl:value-of
																				select="/xlink:httpQuery/queryRequest/criteria" />
																			<br />
																			Result Class:
																			<xsl:value-of
																				select="/xlink:httpQuery/queryRequest/query/class" /> &nbsp;&nbsp;
																		</td>
																	</tr>
																					...
																					<xsl:choose>
																						<xsl:when test="$recordCounter > 0">
																							<td class="dataPagingText" align="right">
																								<xsl:for-each
																									select="/xlink:httpQuery/queryResponse/previous">
																								...
																								of
																								<xsl:value-of select="$recordCounter" />
																								|
																								<xsl:for-each
																									select="/xlink:httpQuery/queryResponse/next">
																									<a class="dataPagingLink" href="{@xlink:href}">Next &gt;&gt;</a>
																								</xsl:for-each>
																							</td>
																						</xsl:when>
																						<xsl:otherwise>
																							<td class="dataPagingText" align="left"
																								style="border:0px; border-bottom:1px; border-style:solid; border-color:#5C5C5C;">
																								<br />
																								Results: Zero records found
																								<br />
																								<br />
																							</td>
																						</xsl:otherwise>
																	...
																	<tr>
																		<td>
																			<table summary="Data Summary" cellpadding="3"
																				cellspacing="0" border="0" class="dataTable" width="100%">
																				<xsl:apply-templates
																					select="/xlink:httpQuery/queryResponse/class" mode="res" />
																			</table>
																		</td>
																	</tr>
	...
	<xsl:template name="request" match="/xlink:httpQuery/queryRequest"
		mode="req">
				...
				<table bgcolor="#98B7B7">
					<tr>
						<td>
							<font color="#003333" size="4">
								Criteria:
								<xsl:value-of select="/xlink:httpQuery/queryRequest/criteria" />
								<br />
							</font>
						</td>
					</tr>
					<tr>
						<td>
							<font color="#003333" size="4">
								Result class name:
								<xsl:value-of select="/xlink:httpQuery/queryRequest/query/class" />
								<br />
							</font>
						</td>
					</tr>
	...
	<xsl:template name="response" match="/xlink:httpQuery/queryResponse/class"
		mode="res">
					...
					<xsl:otherwise>
						<xsl:call-template name="cacore">
							...
							<xsl:with-param name="rec"
								select="following::node()/@recordNumber" />
						</xsl:call-template>
					</xsl:otherwise>
				</xsl:choose>
	...
	<xsl:template name="previous"
		match="/xlink:httpQuery/queryResponse/previous" mode="previous">
	...
	<xsl:template name="page" match="/xlink:httpQuery/queryResponse/pages"
		mode="page">
		<xsl:choose>
	...
	<xsl:template name="next" match="/xlink:httpQuery/queryResponse/next"
		mode="next">
